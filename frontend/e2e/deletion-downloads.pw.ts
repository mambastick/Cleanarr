...
import { boot, clone, downloadItem, fixtureMovie, runtimeConfig, safePlan } from "./fixtures"
...
test("single deletion retries the exact serialized request and prevents rapid duplicate submission", async ({ page }) => {
  ...
  await page.getByRole("tab", { name: "Library" }).click(); await page.getByRole("tab", { name: "Movies" }).click(); await page.getByRole("button", { name: "Delete movie" }).click()
  ...
  const bodies = api.requests.filter((request) => request.pathname === "/api/actions/delete/jobs" && request.method === "POST").map((request) => request.body); expect(bodies[0]).toBe(bodies[1]); const requests = bodies.map((body) => JSON.parse(body ?? "{}")); expect(requests[0].confirmed_plan_hash).toBe("fixture-plan"); expect(typeof requests[0].idempotency_key).toBe("string"); expect(requests[0].idempotency_key).not.toBe(""); expect(requests[0].idempotency_key).toBe(requests[1].idempotency_key)
})
...
test("single deletion exposes preparing, preview retry, accepted-job identity, and portal focus restoration", async ({ page }) => {
  ...
  await page.getByRole("tab", { name: "Library" }).click(); await page.getByRole("tab", { name: "Movies" }).click(); const trigger = page.getByRole("button", { name: "Delete movie" }); await trigger.click()
...
test("Downloads stays cold until active, refreshes, and sends one safe pause request", async ({ page }) => {
  ...
  expect(api.count("/api/downloads")).toBe(0); await page.getByRole("tab", { name: /downloads/i }).click(); await expect.poll(() => api.count("/api/downloads")).toBeGreaterThan(0)
...
test("Downloads retries a structured first-load failure, renders unknown evidence, and resumes exactly once", async ({ page }) => {
  ...
  expect(api.count("/api/downloads")).toBe(0); await page.getByRole("tab", { name: /downloads/i }).click(); await expect(page.getByRole("button", { name: "Retry" })).toBeVisible(); await page.getByRole("button", { name: "Retry" }).click(); await expect(page.getByText("Unknown fixture")).toBeVisible(); await expect(page.getByText("Unknown").first()).toBeVisible(); await expect(page.getByRole("button", { name: "Controls" }).first()).toBeDisabled()
...
test("Downloads pauses polling while hidden and reloads immediately when visible", async ({ page }) => {
  ...
  await page.getByRole("tab", { name: /downloads/i }).click(); await expect.poll(() => api.count("/api/downloads")).toBeGreaterThan(0)
