...
test("setup dialog traps focus, keeps advanced settings compact, and hides the fallback scrollbar", async ({ page }, testInfo) => {
  ...
  await expect(dialog).toBeVisible(); await expect(page.getByRole("button", { name: /skip for now/i })).toBeFocused(); const viewport = dialog.getByRole("region", { name: /first-time setup/i }); await expect(viewport).toHaveCSS("scrollbar-width", "none"); await expect(dialog.locator('[data-slot="stepper"] > div')).toHaveCSS("overflow", "hidden"); await page.waitForTimeout(450); await testInfo.attach("setup-general-compact-followup", { body: await page.screenshot(), contentType: "image/png" }); await page.getByRole("tab", { name: "SSO" }).click(); await expect(page.locator("#wizard-sso-issuer")).toHaveCount(0); await expect(page.getByText("SSO fields are disabled while auth mode is Password-only.", { exact: true })).toBeVisible(); await page.waitForTimeout(250); await testInfo.attach("setup-sso-compact-followup", { body: await page.screenshot(), contentType: "image/png" }); await page.keyboard.press("Tab"); await expect(dialog.locator(":focus")).toHaveCount(1)
...
test("settings Select is a themed portal and keeps English/Russian copy localized", async ({ page }) => {
  ...
  await expect(page.getByRole("region", { name: "Radarr" })).toBeVisible()
  await expect(page.getByRole("region", { name: "Sonarr" })).toBeVisible()
  await expect(page.getByRole("region", { name: "Torrent client" })).toBeVisible()
  await page.getByRole("button", { name: "CleanArr" }).click()
