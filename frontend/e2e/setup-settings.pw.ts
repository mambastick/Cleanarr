...
test("setup dialog traps focus, keeps advanced settings compact, and hides the fallback scrollbar", async ({ page }, testInfo) => {
  ...
  await expect(dialog).toBeVisible(); await expect(page.getByRole("button", { name: /skip for now/i })).toBeFocused(); const viewport = dialog.getByRole("region", { name: /first-time setup/i }); await expect(viewport).toHaveCSS("scrollbar-width", "none"); await expect(page.locator('[data-reduced-motion] > div').first()).toHaveCSS("overflow", "hidden"); await testInfo.attach("setup-general-compact-followup", { body: await page.screenshot(), contentType: "image/png" }); await page.getByRole("tab", { name: "SSO" }).click(); await expect(page.locator("#wizard-sso-issuer")).toHaveCount(0); await expect(page.getByText("SSO fields are disabled while auth mode is Password-only.", { exact: true })).toBeVisible(); await testInfo.attach("setup-sso-compact-followup", { body: await page.screenshot(), contentType: "image/png" }); await page.keyboard.press("Tab"); await expect(dialog.locator(":focus")).toHaveCount(1)
