...
import { boot, clone, fixtureMovie, runtimeConfig, safePlan } from "./fixtures"
...
test("batch preview binds a hash, shows blocked children, requires exact count, and submits once", async ({ page }) => {
  ...
  await page.getByRole("tab", { name: "Library" }).click(); await page.getByRole("tab", { name: "Movies" }).click(); await page.getByRole("checkbox", { name: /select: fixture movie$/i }).check(); await page.getByRole("checkbox", { name: /select: fixture movie two/i }).check(); const trigger = page.getByRole("button", { name: /review batch/i }); await trigger.click()
  ...
  await page.getByRole("button", { name: "Close" }).click(); await expect(trigger).toBeFocused(); const jobs = page.getByRole("button", { name: "Background tasks" }); await jobs.click(); await expect(page.getByText(/partial/i).last()).toBeVisible(); await expect(page.getByText("Fixture Movie Two").last()).toBeVisible(); await page.getByRole("button", { name: "Close" }).last().click(); await expect(jobs).toBeFocused(); const request = api.last("/api/actions/delete/batches", "POST"); expect(request?.body).toContain("fixture-batch-hash")
})
