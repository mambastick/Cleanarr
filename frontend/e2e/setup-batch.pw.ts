...
test("downloader wizard uses client-specific test routes and invalidates visible readiness after a draft edit", async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole("heading", { name: "Downloader" })).toBeVisible(); await expect(page.getByRole("button", { name: "Save" })).toHaveCount(0); await expect(page.getByText("Saves on continue").last()).toBeVisible(); await page.waitForTimeout(450); await testInfo.attach("setup-downloader-followup", { body: await page.screenshot(), contentType: "image/png" })
...
test("batch preview binds a hash, shows blocked children, requires exact count, and submits once", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Close" }).click(); await expect(page.getByRole("button", { name: "Select" })).toBeFocused(); const jobs = page.getByRole("button", { name: "Background tasks" }); await jobs.click(); const jobsDialog = page.getByRole("dialog", { name: "Background tasks" }); await expect(jobsDialog.getByText(/partial/i).last()).toBeVisible(); await expect(jobsDialog.getByText("Fixture Movie Two").last()).toBeVisible(); await jobsDialog.getByRole("button", { name: "Close" }).click(); await expect(jobs).toBeFocused(); const request = api.last("/api/actions/delete/batches", "POST"); expect(request?.body).toContain("fixture-batch-hash"); expect(request?.body).toContain("library-v1:radarr:fixture:101")
})
