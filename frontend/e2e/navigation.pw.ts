...
test("uses isolated startup mocks, keyboard tabs, theme modes, and accessible desktop/mobile shells", async ({ page }) => {
  ...
  await page.getByRole("button", { name: /Theme.*Dark/i }).click(); await expect(page.getByRole("button", { name: /Theme.*System/i })).toBeVisible(); await page.locator(".app-shell__sheet-backdrop").click({ position: { x: 700, y: 100 } }); await expect(account).toBeFocused()
