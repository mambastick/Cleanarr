...
export function UsersPanel({ active, language, currentUsername, currentRole, fetchJson, onCurrentRoleChange }: { active: boolean; language: UsersLanguage; currentUsername: string | null; currentRole: UserRole | null; fetchJson: FetchJson; onCurrentRoleChange?: (role: UserRole) => void }) {
  ...
  return <section className="space-y-5">
    ...
    <Card>
      <CardHeader className="gap-4 sm:flex-row sm:items-center sm:justify-between"><div><CardTitle className="text-base">{text.title}</CardTitle><CardDescription>{users.length}</CardDescription></div><div className="relative w-full sm:max-w-xs"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={text.search} aria-label={text.search} className="pl-9" /></div></CardHeader>
      <CardContent className="p-0">
        ...
        {loading && !users.length ? <div className="space-y-2 p-4" role="status" aria-label={text.loading}><Skeleton className="h-12 w-full" /><Skeleton className="h-12 w-full" /></div> : filtered.length ? <Table>
          ...
          <TableBody>{filtered.map((user) => {
            const isCurrent = user.username.toLocaleLowerCase() === currentUsername?.toLocaleLowerCase()
            return <TableRow key={user.username}><TableCell><div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-full bg-primary/10 text-primary">{user.role === "admin" ? <ShieldCheck className="size-4" /> : <UserRound className="size-4" />}</span><div><p className="font-medium">{user.username}</p>{isCurrent ? <Badge variant="outline" className="mt-1">{text.current}</Badge> : null}</div></div></TableCell><TableCell><Select items={{ admin: text.admin, viewer: text.viewer }} value={user.role} disabled={currentRole !== "admin" || saving === user.username} onValueChange={(value) => void updateRole(user, value as UserRole)}><SelectTrigger className="w-40" aria-label={`${text.role}: ${user.username}`}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="admin">{text.admin}</SelectItem><SelectItem value="viewer">{text.viewer}</SelectItem></SelectContent></Select>{user.role === "admin" && users.filter((candidate) => candidate.role === "admin").length === 1 ? <p className="mt-1 max-w-40 text-xs text-muted-foreground">{text.lastAdmin}</p> : null}</TableCell><TableCell><Badge variant="secondary">{user.auth_source === "local" ? text.local : text.sso}</Badge></TableCell><TableCell>{formatDate(user.last_seen_at, language, text.never)}</TableCell><TableCell>{formatDate(user.created_at, language, text.never)}</TableCell></TableRow>
          })}</TableBody>
