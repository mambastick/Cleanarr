...
import { actionDescription, actionTitle, type ActivityLanguage } from "./action-presentation"

type ActivityItem = DashboardUnifiedActivityItem

export function ActivityPanel({ text, language = "en", filteredActivity, webhookAttempts, activityFilter, onFilterChange }: { text: UiTextMap; language?: ActivityLanguage; filteredActivity: DashboardActivity[]; webhookAttempts: DashboardWebhookAttempt[]; activityFilter: string; onFilterChange: (value: string) => void }) {
  ...
  return <section className="space-y-5">
    ...
    <Card className="overflow-hidden">
      ...
      <CardContent className="p-0">{activityItems.length === 0 ? <div className="p-5"><EmptyState title={text.noActivity} description={activityFilter ? text.noActivityFiltered : text.sendWebhookToSeeActivity} /></div> : <div role="list" aria-label={text.activityTimeline} className="divide-y">{activityItems.map((item) => <ActivityEventRow key={activityKey(item)} item={item} text={text} language={language} />)}</div>}</CardContent>
    </Card>
  </section>
}
...
function ActivityEventRow({ item, text, language }: { item: ActivityItem; text: UiTextMap; language: ActivityLanguage }) {
  ...
  return <details role="listitem" className="group open:bg-muted/20">
    ...
    <div className="border-t bg-background/60 px-4 py-4 sm:pl-[4.25rem]"><p className="mb-3 text-xs text-muted-foreground md:hidden">{new Date(timestamp).toLocaleString()}</p>{processed ? <ProcessedDetails entry={item} text={text} language={language} /> : <WebhookDetails attempt={item} text={text} />}</div>
  </details>
}

function ProcessedDetails({ entry, text, language }: { entry: DashboardActivity; text: UiTextMap; language: ActivityLanguage }) {
  return <div className="space-y-2">{entry.result.actions.length ? entry.result.actions.map((action, index) => <ActionDetail key={`${action.system}-${action.action}-${index}`} action={action} text={text} language={language} />) : <p className="text-sm text-muted-foreground">{text.noItemsYet}</p>}</div>
}
...
function ActionDetail({ action, text, language }: { action: DashboardAction; text: UiTextMap; language: ActivityLanguage }) {
  return <div className="rounded-lg border bg-card p-3"><div className="flex flex-wrap items-start justify-between gap-2"><p className="min-w-0 flex-1 text-sm font-medium text-foreground">{actionTitle(action, language)}</p><StatusPill tone={action.status === "failed" ? "red" : action.status === "deleted" || action.status === "already_absent" ? "green" : "blue"} label={getStatusLabel(action.status, text)} /></div><p className="mt-2 text-xs leading-relaxed text-muted-foreground">{actionDescription(action, language)}</p></div>
}
