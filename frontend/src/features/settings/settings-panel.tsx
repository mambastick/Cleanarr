...
export function SettingsPanel({ config, isConfigLoading, onSaveGeneral, onAddService, onEditService, text, language, settingsSection = "cleanarr" }: SettingsPanelProps) {
  ...
  return <section className="space-y-5">
    ...
    {settingsSection === "services" ? <Card><CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-base"><Server className="size-4 text-status-success" />{text.connectedServices}</CardTitle><CardDescription>{text.allEnabledRouting}</CardDescription></CardHeader><CardContent className="divide-y overflow-hidden rounded-xl border p-0">
      {SERVICE_FAMILIES.map((family) => {
        ...
        return <section key={family} className="grid items-center gap-4 p-4 lg:grid-cols-[minmax(12rem,.8fr)_minmax(0,1.7fr)_auto]">
          <div><p className="text-sm font-semibold">{familyTitle}</p><p className="mt-1 text-xs text-muted-foreground">{getServiceDescription(family, text)}</p></div>
          <div className="min-w-0 divide-y lg:border-x lg:px-4">{services.length ? services.map((service) => <div key={service.id} className="flex flex-wrap items-center gap-2 py-2 first:pt-0 last:pb-0"><div className="mr-auto min-w-0"><p className="truncate text-sm font-medium">{service.name}</p><p className="truncate text-xs text-muted-foreground">{family === "downloaders" ? getDownloaderLabel(service.kind as DownloaderKind) : familyTitle} · {service.url}</p></div>{service.is_default ? <Badge variant="outline">{text.defaultLabel}</Badge> : null}<StatusPill tone={service.enabled ? "green" : "blue"} label={service.enabled ? text.enabled : text.disabled} /><Button variant="ghost" size="sm" onClick={(event) => onEditService(family, service, event.currentTarget)}>{text.edit}</Button></div>) : <p className="py-2 text-xs text-muted-foreground">{text.notConfigured}</p>}</div>
          <Button className="self-center justify-self-start lg:justify-self-end" variant="outline" size="sm" onClick={(event) => onAddService(family, event.currentTarget)}><Plus className="size-4" />{text.add}</Button>
        </section>
      })}
