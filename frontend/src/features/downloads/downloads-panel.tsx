...
export function DownloadsPanel({ active, authenticated, language, isLive, fetchJson, onActiveCountChange, onDelete, onBatchPreview }: { active: boolean; authenticated: boolean; language: DownloadsLanguage; isLive: boolean; fetchJson: FetchJson; onActiveCountChange: (count: number) => void; onDelete: (target: LibraryDeleteTarget, trigger: HTMLElement) => void; onBatchPreview: (items: BatchSelectionItem[], trigger: HTMLElement) => void }) {
  ...
  const control = useCallback(async (item: DownloadItem, action: DownloadAction) => {
    ...
    const body = previous?.body ?? JSON.stringify({ client_id: item.client_id, info_hash: item.info_hash, action, idempotency_key: crypto.randomUUID() })
